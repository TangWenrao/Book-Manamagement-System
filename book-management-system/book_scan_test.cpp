...
int book_struct_test(FILE*p,int amount){//结构体调试函数 
	int i=0;
	for(i=0;i<amount;i++){
		book_print(p,i);
	} 
...
char book_scan(){//图书查找函数 
	...
	amount=book_amount(p);
	if(book[amount].book_name==NULL){
		book_struct2(p2,amount);
		printf("导入结构体2\n");
	}
	if(book_ck[amount].book_name==NULL){
